trigger scaleMattersLeadTrigger on Lead (after Delete, after Update) {
    ...
    List<scaleMatters__CycleMapping__c> cycleList = [ select scaleMatters__Lead_Cycle_Date__c from scaleMatters__CycleMapping__c where scaleMatters__Cycle_Name__c Like 'scaleMatters Cycle Mapping' AND Isdeleted = false];
    ID leadObjId;
    ...
    if(scaleMattersUpdateMappedDates.firstRun){  
     ...
     scaleMattersUpdateMappedDates.updateSMRecords('Lead', leadIdsList , oldMappedDate , newMappedDate );
